...
<head>
    ...
    <title>Management & Balance Lab</title>
    <style>
        :root {
        ...
        .gap-input { border: none; border-bottom: 2px solid var(--secondary-color); width: 180px; text-align: center; font-size: inherit; outline: none; background: transparent; color: var(--accent-color); font-weight: bold; }
        ...
        .match-box.selected { border-color: var(--secondary-color); background: #fef5e7; }
...
<div class="container">
    ...
    <div id="tab-1" class="exercise-section" style="display:none">
        <h2>✍️ Interactive Sentence Writing</h2>
        <div id="writing-bank" class="word-bank"></div>
        <div id="writing-content"></div>
        <button class="action-btn" onclick="checkWriting()">Check Progress</button>
    </div>

    <div id="tab-2" class="exercise-section" style="display:none">
        <h2>🔗 Meaning-to-Word Match</h2>
...
<script>
    const data = [
        { word: "do for a living", def: "What a person does as their job or profession.", sents: ["I don't know what they ___, but they have a nice car.", "What do you want to ___ when you grow up?"] },
        { word: "overwhelmed", def: "Feeling like you have too much to handle.", sents: ["I feel ___ with all this new work.", "Don't get ___, just take it one step at a time."] },
        { word: "timeframe", def: "The period of time during which something happens.", sents: ["What is the ___ for completing this project?", "We need a realistic ___ to finish the repairs."] },
        { word: "tick them off", def: "To mark items on a list as finished.", sents: ["I have five tasks left to ___.", "It is satisfying to ___ your chores."] },
        { word: "meanwhile", def: "At the same time; in the intervening period.", sents: ["The coffee is brewing; ___, I'll make toast.", "She was studying; ___, her brother was playing."] },
        { word: "salary", def: "Monthly or yearly pay from a job.", sents: ["They offered me a very competitive ___.", "Does the ___ include bonuses?"] },
        { word: "improve", def: "To get better at something.", sents: ["Practice every day to ___ your accent.", "I want to ___ my productivity."] }
    ];
    ...
    function init() {
        ...
        shuffle(pPool);
        document.getElementById('pron-content').innerHTML = pPool.map((s, i) => `<div class="gap-sentence">${s.t.replace(s.a, '_________')} <button class="mic-btn" onclick="testSpeech('${s.a}', 'p-fb-${i}')">🎤</button><div id="p-fb-${i}" style="font-size: 0.85em; color:#888; margin-top:5px;">Check the bank & speak!</div></div>`).join('');
    }
